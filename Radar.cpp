...
class Radar : public bz_Plugin
{
   public:
   const char* Name(){return "Radar";}
...
void Radar::Init (const char* commandLine) {
    for(int i=0;i<=199;i++){
        grabbed[i]=0;
   ...
   //radarRange;
   bz_registerCustomBZDBDouble("_radarRange", bz_getBZDBDouble("_worldSize"));
   bz_RegisterCustomFlag("RD", "RADAR", "Enables radar when there is none or at larger range.", 0, eGoodFlag);
}

void Radar::Cleanup (void) {
   bz_removeCustomBZDBVariable("_radarRange");
   Flush();
}
...
void Radar::Event(bz_EventData *eventData ) {
        ...
        for(int i=0; i<=199;i++) {
            if (grabbed[i] == 1) {
                if (checkIfValidPlayer(i) == true) {
                    sendPlayerBZDB(i,"_radarLimit", bz_getBZDBDouble("_radarRange"));
                }
            }
        }
      } else {
...
// (double)       eventTime - This value is the local server time of the event.
    ...
    case bz_eFlagGrabbedEvent: {
      ...
      if (checkPlayerSlot(player)==1) {
          if (strcmp(flagGrabData->flagType, "RD") == 0) {
            sendPlayerBZDB(player,"_radarLimit", bz_getBZDBDouble("_radarRange"));//radarRange);//800.0);//
...
// (double)       eventTime - This value is the local server time of the event.
    ...
    case bz_eFlagDroppedEvent: {
      ...
      if (checkPlayerSlot(player)==1) {
          if (strcmp(flagDropData->flagType, "RD") == 0) {
...
// (double)       eventTime - This value is the local server time of the event.
    ...
    case bz_eFlagTransferredEvent: {
      ...
      if (strcmp(thiefData->flagType, "RD") == 0) {
        sendPlayerBZDB(from,"_radarLimit", bz_getBZDBDouble("_radarLimit"));
        if (checkPlayerSlot(from)==1) {
            grabbed[from]=0;
        }
        
        sendPlayerBZDB(to,"_radarLimit", bz_getBZDBDouble("_radarRange"));//radarRange);
